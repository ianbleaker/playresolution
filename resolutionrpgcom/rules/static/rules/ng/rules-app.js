(function(){
    ...
    app.controller("ContentController", ['$scope', '$http', function($scope, $http){
        ...
        $scope.functions.ctrl.get = function(contentTitles, args){
            //null args handler
            args = args || {};
            //if arg is just function, make it after all
            ...
            //handle before or after being a function
            if (typeof args.before == 'function') args.before = {all: args.before};
            if (typeof args.after == 'function') args.after = {all: args.after};

            //set up function runner
            function runner(when, content){
                //abort if nothing here
                if(!args[when]) return;
                //if not content provide, assume "all"
                ...
                setTimeout(function(){
                    ...
                    //otherwise we're done, run the after stuff
                        ...
                        if(args.toast && content == 'all') {
                            $.Velocity($('.' + slug), {"opacity": 0, marginTop: '-40px'}, { duration: 375,
                                easing: 'easeOutExpo',
                                queue: false,
                                complete: function(){
        ...
        $scope.functions.ctrl.organizeByCategory = function(data, categoryName){
            ...
            for (var i = 0; i < data.length; i++){
                ...
                outputObject[data[i][categoryName]].content.sort(function(a,b) {return (a.name > b.name) ? 1 : ((b.name > a.name) ? -1 : 0);} );
            }
            ...
            output.sort(function(a,b) {return (a.name > b.name) ? 1 : ((b.name > a.name) ? -1 : 0);} );
